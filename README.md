...
**【モデル選定と最適化】**
開発当初は、ローカルLLM「Swallow-7b」のファインチューニングを計画していましたが、「推論速度」と「リソース」の課題に直面しました。そこで、ユーザー体験（UX）を最優先するために **Gemini 1.5 Flash API** へ移行。さらに、長文規約を扱うためにテキストをチャンク分割し、**並列処理** でAPIを叩くことで、解析時間を大幅に短縮しました。
